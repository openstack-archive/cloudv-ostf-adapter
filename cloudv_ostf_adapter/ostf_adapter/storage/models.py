...
from cloudv_ostf_adapter.ostf_adapter import nose_plugin
from cloudv_ostf_adapter.ostf_adapter.storage import engine
from cloudv_ostf_adapter.ostf_adapter.storage import fields
...
class TestRun(BASE):
    ...
    @classmethod
    def add_test_run(cls, session, test_set, status='running', tests=None):
        ...
        tests_in_set = session.query(TestSet).filter_by(
            id=test_set).one().tests
        tests_names = [t.name for t in tests_in_set]
        ...
        test_run = cls(test_set_id=test_set, status=status)
    ...
    @classmethod
    def get_last_test_run(cls, session, test_set):
    ...
    @classmethod
    def is_last_running(cls, session, test_set):
        ...
        """
        test_run = cls.get_last_test_run(session, test_set)
    ...
    def start(cls, session, test_set, metadata, tests, dbpath, token=None):
        ...
        if cls.is_last_running(session, test_set.id):
            test_run = cls.add_test_run(session, test_set.id, tests=tests)
            ...
            return cls.get_last_test_run(session, test_set.id)
    ...
    def restart(self, session, dbpath,
        ...
        """
        if TestRun.is_last_running(session, self.test_set_id):
