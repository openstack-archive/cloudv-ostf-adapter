...
class ValidationPlugin(object):
    ...
    def run_suite(self, suite, db_test=None):
        result = self._run_suite(suite)
        if db_test is not None:
            self.store_result_to_db(db_test, result)

    def store_result_to_db(self, db_test, result):
        with engine.contexted_session(cfg.CONF.rest.dbpath) as session:
